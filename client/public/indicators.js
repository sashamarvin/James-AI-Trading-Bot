...
function integrateEMA(stockData) {
    const ema10Data = calculateEMA(stockData, 10);
    const ema50Data = calculateEMA(stockData, 50); // 🆕 added

    stockData.forEach((bar, i) => {
        bar.ema10 = ema10Data[i]?.value || null;
        bar.ema50 = ema50Data[i]?.value || null; // 🆕 injected
    });
}
...
// ✅ Compute and Plot EMA50
function calculateAndPlotEma50(stockData, chart, allSeries) {
    const ema50Data = calculateEMA(stockData, 50);

    const ema50Series = chart.addSeries(LightweightCharts.LineSeries, {
        color: 'rgba(231, 76, 60, 0.7)', // ✅ Soft Red, 70% opacity
        lineWidth: 1,
        lineStyle: LightweightCharts.LineStyle.Solid,
    });

    ema50Series.setData(ema50Data);
    allSeries.push(ema50Series);

    // ✅ Disable Hover Dots for EMA50
    ema50Series.applyOptions({
        crosshairMarkerVisible: false, // 🚀 Extra check
    });

    //console.log("📊 EMA50 Plotted →", ema50Data.slice(0, 5));
}
