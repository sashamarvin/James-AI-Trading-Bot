document.addEventListener("DOMContentLoaded", function () {
    ...
    chart.subscribeClick(param => {
        ...
        if (currentMode === "stopLineMode" && drawMode) {
            const time = param.time;
            const stopPrice = cursorPrice; // ✅ Save the selected stop price
        
            // ✅ Find the index of the clicked day
            const chartData = mainSeries.data();
            let stopIndex = chartData.findIndex(d => d.time === time);
            
            if (stopIndex <= 0) return; // Prevent errors

            let startIndex = stopIndex - 1; // ✅ Start one trading day before
            let endIndex = stopIndex + 1;   // ✅ End one trading day after
            ...
            // ✅ Create a new line series for this stop point
            const stopLineSeries = chart.addSeries(LightweightCharts.LineSeries, {
                color: 'red',
                lineWidth: 2,
                lineStyle: LightweightCharts.LineStyle.Solid,
            });

            stopLineSeries.setData([
                { time: startTime, value: cursorPrice }, // Start one day before
                { time: endTime, value: cursorPrice }   // End one day after
            ]);

            // ✅ Store the new stop line in the global `stopLines` array
            stopLines.push(stopLineSeries);

            // ✅ Place stop marker below the candle
            stopMarkers.push({
                time: time,
                position: "belowBar",
                color: "red",
            ...
            // ✅ First, push stopPoint to the array
            stopPoints.push({ time, value: stopPrice });

            // ✅ Delay risk calculation slightly to ensure `stopPoints` is updated
            setTimeout(() => {
                if (stopPoints.length > 0) {
                    calculateRiskPercentage();
                }
        ...
        if (currentMode === "lambdaMode") {
            if (!leftDay) {
                // Set left day on first click
                leftDay = param.time;
                console.log("🔵 Left day selected:", leftDay);
            } else if (!rightDay) {
                // Set right day on second click
                rightDay = param.time;
                console.log("🔵 Right day selected:", rightDay);
    
                // Find the index of left and right days in the stockData
                const startIdx = stockData.findIndex(d => d.time === leftDay);
                const endIdx = stockData.findIndex(d => d.time === rightDay);
    
                console.log("🔵 Start Index:", startIdx, "End Index:", endIdx);
    
                if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
                    // Find alternating highs and lows between the two days
                    const zigZagPoints = find_alternating_highs_lows(stockData, startIdx, endIdx);
                    
                } else {
                    console.error("❌ Invalid start or end index.");
                }
    
                // Reset the days after selecting right day
                leftDay = null;
                rightDay = null;
            }
        }
        
        if (currentMode === "lambdaModeFlatOrAscending") {
            if (!leftDay) {
                leftDay = param.time;
                console.log("🟣 Flat/Asc Left day:", leftDay);
            } else if (!rightDay) {
                rightDay = param.time;
                console.log("🟣 Flat/Asc Right day:", rightDay);
        
                const startIdx = stockData.findIndex(d => d.time === leftDay);
                const endIdx = stockData.findIndex(d => d.time === rightDay);
        
                if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
                    const pattern = find_flat_or_ascending_highs_lows(stockData, startIdx, endIdx);
                } else {
                    console.error("❌ Invalid flat/asc start or end index.");
                }
        ...
        if (currentMode === "lambdaModeHybrid") {
            if (!leftDay) {
                leftDay = param.time;
                console.log("🧠 Hybrid Left day:", leftDay);
            } else if (!rightDay) {
                rightDay = param.time;
                console.log("🧠 Hybrid Right day:", rightDay);
        
                const startIdx = stockData.findIndex(d => d.time === leftDay);
                const endIdx = stockData.findIndex(d => d.time === rightDay);
        
                if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
                    const hybrid = find_lambda_hybrid(stockData, startIdx, endIdx);
                    // Optional: do something with `hybrid`
                } else {
                    console.error("❌ Invalid hybrid start or end index.");
                }
    ...
    document.addEventListener("keydown", (event) => {
        if (event.key === "Delete" || (event.key === "Backspace" && !event.metaKey && !event.ctrlKey)) {
            ...
            if (window.currentMode === "breakoutLineMode" && drawMode) {
                if (breakoutPoints.length > 0) {
                    console.log("🗑 Removing Last Breakout Point...");
                    breakoutPoints.pop(); // ✅ Remove last breakout point
            
                    // ✅ Remove last breakout trendline if it exists
                    if (breakoutTrendlines.length > 0) {
                        const lastLine = breakoutTrendlines.pop();
                        chart.removeSeries(lastLine);
                    }
            
                    // ✅ Reset status if no breakout points remain
                    if (breakoutPoints.length === 0) {
                        while (breakoutTrendlines.length > 0) {
                            chart.removeSeries(breakoutTrendlines.pop());
                        }
                        resetBreakoutLineStatus();
                    }
                }
            } else if (window.currentMode === "breakoutLineMode" && !drawMode) {
                console.log("🗑 Removing All Breakout Lines...");
                ...
                // ✅ Remove the little blue breakout line
                while (oneDayLines.length > 0) {
                    chart.removeSeries(oneDayLines.pop());
                }
            }

            if (window.currentMode === "stopLineMode" && drawMode) {
                if (stopMarkers.length > 0) {
                    console.log("🗑 Removing Last Stop Marker...");
                    stopMarkers.pop(); // ✅ Remove last stop marker
                    LightweightCharts.createSeriesMarkers(mainSeries, stopMarkers);
            
                    // ✅ Remove last stop line if one exists
                    if (stopLines.length > 0) {
                        const lastLine = stopLines.pop(); // ✅ Remove from array
                        chart.removeSeries(lastLine); // ✅ Remove from chart
                    }
            
                    // ✅ Reset status if no stop markers remain
                    if (stopMarkers.length === 0) {
                        // ✅ Remove all stop markers without affecting other markers
                        while (stopMarkers.length > 0) {
                            stopMarkers.pop();
                        }
                        // ✅ Update chart with only stop markers
                        LightweightCharts.createSeriesMarkers(mainSeries, stopMarkers);
                        // ✅ Remove all stop lines from the chart
                        while (stopLines.length > 0) {
                            const line = stopLines.pop(); // ✅ Remove from array
                            chart.removeSeries(line);     // ✅ Remove from chart
                        }
                        resetStopLineStatus();
                    }
                }
            } else if (window.currentMode === "stopLineMode" && !drawMode) {
                // ✅ Remove all stop markers without affecting other markers
                while (stopMarkers.length > 0) {
                    stopMarkers.pop();
                }
    ...
    */
    ...
    // Toggle lambdaMode to descending when "Q" is pressed
    document.addEventListener("keydown", (e) => {
        if (e.key === "q" || e.key === "Q") {
            window.currentMode = "lambdaMode";
            console.log("🔵 lambdaMode activated");
        }
    });
    // Toggle lambdaMode to ascending when "W" is pressed
    document.addEventListener("keydown", (e) => {
        if (e.key === "w" || e.key === "W") {
            window.currentMode = "lambdaModeFlatOrAscending";
            console.log("🔵 lambdaModeFlatOrAscending activated");
        }
    });
    // Toggle lambdaMode Hybrid when "A" is pressed
    document.addEventListener("keydown", (e) => {
        if (e.key === "a" || e.key === "A") {
            window.currentMode = "lambdaModeHybrid";
            console.log("🧠 lambdaModeHybrid activated");
        }
    });
