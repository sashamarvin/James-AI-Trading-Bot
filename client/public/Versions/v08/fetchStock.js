...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const stockInput = document.getElementById("stockInput"); // Lower input box
...
async function fetchStockData() {
  try {
      ...
      console.log(`📊 DEBUG: Total Rows After Filtering: ${rows.length}`);
      ...
      console.log("📊 DEBUG: First 10 Rows (Complete) →");
      parsedData.slice(0, 10).forEach((row, index) => {
          console.log(`#${index + 1} → Ticker: ${selectedTicker}, Time: ${row.time}, Open: ${row.open}, High: ${row.high}, Low: ${row.low}, Close: ${row.close}, Volume: ${row.volume}`);
      });
