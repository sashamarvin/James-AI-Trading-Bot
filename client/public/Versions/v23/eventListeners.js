document.addEventListener("DOMContentLoaded", function () {
    ...
    document.addEventListener("keydown", (event) => {
        if (event.key === "Delete" || (event.key === "Backspace" && !event.metaKey && !event.ctrlKey)) {
            ...
            if (window.currentMode === "insideDayMode") {
                if (insideDayMarkers.length > 0) {
                    console.log("🗑 Removing Last Inside Day Marker...");
            
                    insideDayMarkers.pop(); // ✅ Remove last marker
            
                    LightweightCharts.createSeriesMarkers(mainSeries, insideDayMarkers); // ✅ Update chart
            ...
            if (window.currentMode === "harmonicsLineMode") {
